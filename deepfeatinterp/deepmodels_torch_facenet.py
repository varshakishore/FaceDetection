...
from torch_models import ReconModel
from torch_models_facenet import InceptionResnetV1
...
class Modified_Facenet(nn.Module):
    def __init__(self, model):
        super(Modified_Facenet, self).__init__()
        self.model = model
...
class facenet_torch_base(object):
  def __init__(self, **options):
    device_id = options.get('device_id', 0)
    model = InceptionResnetV1(pretrained='vggface2')
    model.classify = False
    self.forward_model = Modified_Facenet(model)
...
class facenet_torch_recon_base(facenet_torch_base):
  def __init__(self, device_id, drop_rate):
    self.recon_model = ReconModel(drop_rate)
    self.recon_model.eval()
    super(facenet_torch_recon_base, self).__init__(device_id=device_id)
...
class facenet_torch(facenet_torch_base):
  def F_inverse_with_uncertainty(self, F, initial_image):
...
#       criterion5 = nn.MSELoss(size_average = False).cuda()
      ...
      def step():
        ...
        loss1 = sum(criterion(output_var[i], features[i]) for i in range(len(output_var)))
