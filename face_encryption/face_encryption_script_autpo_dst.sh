...
## 1. Find the latent vector
source deactivate 
source activate stylegan2

### 1.1 Crop the image under a certain format
python ../stylegan2-pytorch/alignFace.py --directory $img_directory --input_name $orig_name

### 1.2 white out the background (TODO: NEEDS TO BE REPLACED BY REAL CODES INSTEAD OF API)
python ../stylegan2-pytorch/rm_background.py --directory $img_directory --input_name ${orig_name}_cropped

### 1.3 Find the latent vector and get the projected images
...
## 2. Generate swapped img
python ../stylegan2-pytorch/projector.py --ckpt ../stylegan2-pytorch/stylegan2-ffhq-config-f.pt --step 1000 --size=1024 --output_folder $img_directory/ --mask 300_700_400_850 --mse 10 ${img_directory}/${orig_name}_cropped_no-bg-project.jpg

source deactivate
source activate faceswap
python ../FaceSwap/main.py --src ${img_directory}/${orig_name}_cropped_no-bg-project_mask_300_700_400_850-project.jpg --dst ${img_directory}/${orig_name}.jpg --out ${img_directory}/${orig_name}_encrypt_init.jpg --no_debug_window --correct_color
