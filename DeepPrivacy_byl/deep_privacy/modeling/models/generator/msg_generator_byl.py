...
class MSGGenerator_BYL(RunningAverageGenerator):
    def __init__(
        ...
        self._min_fmap_resolution = min_fmap_resolution #* ((self.divide // 2) ** 2)
    ...
    def _init_encoder(self):
        ...
        while imsize >= self._min_fmap_resolution:
            # print(imsize)
            # print(self._min_fmap_resolution)
            ...
            next_size = self.res2channels[max(imsize // self.divide, self._min_fmap_resolution)]
            ...
            if imsize != self._min_fmap_resolution:
                self.encoder.add_module(
                    f"downsample{imsize}", layers.AvgPool2d(self.divide))
            imsize //= self.divide
    ...
    def _init_decoder(self):
        ...
        while imsize <= self.max_imsize:
            # print(imsize)
            # print(self.max_imsize)
            # import pdb; pdb.set_trace()
            current_size = self.res2channels[max(imsize//self.divide, self._min_fmap_resolution)]
            ...
            if imsize == self._min_fmap_resolution:
                ...
                if self.divide == 4:
                    self.decoder.add_module(f"upsample{imsize}", layers.NearestUpsample_4())
                else:
    ...
    def forward_decoder(self, x, mask, batch):
        ...
        # import pdb; pdb.set_trace()
        self.divide = 4
        imsize_start = max(x.shape[-1] // self.divide, 1)
        rgb = torch.zeros(
        ...
        mask_out = torch.zeros(
            (x.shape[0], mask_size,
             imsize_start, imsize_start),
            dtype=x.dtype, device=x.device)
        imsize = self._min_fmap_resolution // self.divide
        for module in self.decoder:
            ...
            if isinstance(module, blocks.BasicBlock):
                ...
                imsize *= self.divide
                # print(rgb.shape)
                rgb = layers.up(rgb, scale_factor=self.divide)
                mask_out = layers.up(mask_out, scale_factor=self.divide)
    ...
    def forward(
        ...
        # print(batch['z'].shape)
        # import pdb; pdb.set_trace()
        # x=torch.zeros(32,128,4,4).cuda()
        # mask=torch.zeros(32,1,4,4).cuda()
...
# #     import sys
# #     sys.exit()
#     g = MSGGenerator_BYL(max_imsize=128, conv_size = {
#             "4": 512,
#             "8": 512,
#             "16": 512,
#             "32": 512,
