...
class StyleGANInverter(object):
  ...
  def invert(self, image, mask, num_viz=0):
    ...
    if mask is None:
        init_z = self.get_init_code(x)
        def gan_forward(z):
            return self.G.net.synthesis(z)
        #init_z = torch.randn(1, 512)
    else:
        #x = x * mask
        init_z = torch.randn(1, 512)
        def gan_forward(z):
            return self.G.net.synthesis(self.G.net.truncation(self.G.net.mapping(z).view(1, 14, -1)))
...
#         loss_reg = torch.mean((z - z_rec) ** 2) + 0.01 * torch.abs(torch.norm(z) - np.sqrt(len(z)))
...
#         log_message += f', loss_reg: {_get_tensor_value(loss_reg):.3f}'
    ...
    if mask is not None:
        viz_results.append(self.G.postprocess(_get_tensor_value(x * mask))[0])

    return _get_tensor_value(z), viz_results

  def easy_invert(self, image, mask=None, num_viz=0):
    """Wraps functions `preprocess()` and `invert()` together."""
    return self.invert(self.preprocess(image), mask, num_viz)
  ...
  def diffuse(self,
    ...
    for step in pbar:
      ...
      # Reconstruction loss.
      x_rec = self.G.net.synthesis(z)
      loss_pix = torch.mean(((x - x_rec) * (1 - mask)) ** 2, dim=[1, 2, 3])
      loss = loss + loss_pix * self.loss_pix_weight
